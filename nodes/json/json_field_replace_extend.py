...
class JsonFieldReplaceExtend:
    ...
    Handles non-string inputs (lists, dicts, numbers) directly.
    """
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "value": (
                "*", {"default": "", "tooltip": "Value to set (accepts STRING, LIST, DICT, INT, FLOAT, BOOL, NULL)"}),
                "extend_value": (
                "BOOLEAN", {"default": False, "tooltip": "If true, prepend value to existing string/list"}),
            }
        }
    ...
    def _cast_value(value: str):
        """Кастует строку в нативный Python тип."""
        v = value.strip()
        v_low = v.lower()
    ...
    def _is_empty_value(value) -> bool:
        ...
        if isinstance(value, (list, dict)):
            return len(value) == 0
        # Для чисел, bool — не считаем пустыми
        return False

    @staticmethod
    def _set_by_path(data, path_parts, value, extend=False):
        """Устанавливает значение по пути с поддержкой extend для строк и списков."""
        obj = data
            ...
            if part.isdigit():
                ...
                if is_last:
                    if extend and idx < len(obj) and obj[idx] is not None:
                        ...
                        # Extend для списков — конкатенация
                        elif isinstance(existing, list) and isinstance(value, list):
                            obj[idx] = value + existing
                        else:
                            obj[idx] = value
                    else:
                        obj[idx] = value
    ...
    def replace_field(self, json_string, key, value, extend_value):
        # Проверка пустого значения (универсальная для всех типов)
        if self._is_empty_value(value):
            log(LogEntry(
                node_class="JsonFieldReplaceExtend",
                title="Skipped",
                details={"Reason": "Empty value"},
                footer="Returning original JSON"
            ))
            return (json_string,)
        ...
        try:
            ...
            try:
                ...
                log(LogEntry(
                    node_class="JsonFieldReplaceExtend",
                    title="Parse failed",
                    details={"Error": str(e), "Fallback": str(fallback_err)},
        ...
        if not key.strip():
            log(LogEntry(
                node_class="JsonFieldReplaceExtend",
                title="Empty key",
                details={"Action": "Returning formatted JSON"},
                footer="No modification needed"
            ))
            return (json.dumps(data, ensure_ascii=False, indent=2),)

        # Обработка value в зависимости от типа
        if isinstance(value, str):
            # Для строк — пытаемся распарсить как JSON, иначе кастуем
            casted = value
            try:
            ...
            # Для не-строк (list, dict, int, float, bool, None) — используем как есть
            casted = value
        ...
        if not path_parts:
            log(LogEntry(
                ...
                title="Invalid path",
                details={"Key": key},
                footer="Returning formatted JSON"
            ))
            return (json.dumps(data, ensure_ascii=False, indent=2),)
        ...
        log(LogEntry(
            ...
            details={
                "Key": key,
                "Extend": extend_value,
                "Path Length": len(path_parts),
                "Value Type": type(casted).__name__
            },
