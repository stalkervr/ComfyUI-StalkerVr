...
class TextLoadFromDirectoryList:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
    ...
    def load_texts(self, directory_path, extension, mode="single", sort_by="name", separator="\n--\n"):
        ...
        if not files:
            log(LogEntry(node_class="TextLoadFromDirectoryList", title="No files found",
                         details={"Path": str(dir_path), "Ext": extension}))
            return ([],)
        ...
        for file_path in files:
            try:
                ...
                log(LogEntry(node_class="TextLoadFromDirectoryList", title="Skipped file",
                             details={"File": file_path.name, "Error": str(e)}))
                continue

        log(LogEntry(
            node_class="TextLoadFromDirectoryList",
            title="Loaded files",
            details={"Count": len(files), "Mode": mode, "Total Items": len(final_list)}
        ))
